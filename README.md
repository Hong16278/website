...
当前首页采用横向滚动的软件主视觉，集中展示 AutoMate、鼠标助手、定时消息助手、键盘点击助手和定时截图，并使用仓库中的真实软件截图。支持自动轮播、软件标签、左右按钮、键盘方向键和移动端滑动。识别提醒助手与快捷回复助手保留为开发预览。网站不虚构下载量、客户评价或未配置的下载入口。
...
站点可通过 ChatGPT Sites 进行所有者私密预览，也保留 Docker 自托管方案。未经站点所有者明确调整访问权限，不应改为公开访问。
